...
const AllSubscriptions = (props) => {
  ...
  const { notWatched } = useContext(subscriptionsContext);
  ...
  const subscribe = (memberId) => {
    props.movies.forEach((movie) => {
      ...
      if (!flag) {
        setArrayOfNotWatched((arrayOfNotWatched) => [
          ...arrayOfNotWatched,
          movie.Name,
        ]);
      }
  ...
  let allSubscriptions = members.map((member, index) => {
    return (
      <li key={index} className="allSubscriptions">
        <div className="subscription" style={{ height: 300 }}>
          <div className="userDataSubscribe">
            <div className="emailCityBtns">
              <form>
                ...
                <br />
                <Link
                  to={`/main/subscriptionsMenu/editSubscriptions/${member.Name}`}
                >
                  <button className="btn">Edit</button>
                </Link>
                <Link
                  to={`/main/subscriptionsMenu/deleteSubscriptions/${member.Name}`}
                >
          ...
          <br />
          <div
            className="SubscriptionsWatched"
            style={{
          ...
          >
            ...
            <br />
            <Link
              to={`/main/subscriptionsMenu/subscribeToNewMovie/${member.Name}`}
            >
              <button
                className="btn"
                type="button"
            ...
            {props.subscriptions.map((subscription, index) => {
              return (
                <ul className="ulStyle" key={index}>
                  {
                    /*eslint-disable eqeqeq*/
                    props.movies.map((movie, index) => {
                      if (
                        subscription.Movie.movieId == movie._id &&
