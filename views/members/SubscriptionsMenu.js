...
const SubscriptionsMenu = (props) => {
  ...
  return (
    <div style={{ marginTop: 50 }}>
      <ul className="Movies">
        <h3>Subscriptions</h3>
        <Link to="/main/subscriptionsMenu/allSubscriptions">
          <button>All Members</button>
        </Link>
        <Link to="/main/subscriptionsMenu/addSubscriptions">
          <button style={{ marginLeft: 5, marginRight: 5 }}>Add Member</button>
        </Link>
        <Switch>
          <Route
            path="/main/subscriptionsMenu/allSubscriptions"
            render={() => (
              <AllSubscriptions movies={movies} subscriptions={subscriptions} />
            )}
          />
          <Route
            path="/main/subscriptionsMenu/addSubscriptions"
            component={AddSubscriptions}
          />
          <Route
            path="/main/subscriptionsMenu/editSubscriptions/:member"
            render={(props) => (
              <EditMembers
                memberName={props.match.params.member}
                members={members}
              />
            )}
          />
          <Route
            path="/main/subscriptionsMenu/deleteSubscriptions/:member"
            render={(props) => (
              <DeleteMembers
                memberName={props.match.params.member}
                members={members}
          ...
          />
          <Route
            path="/main/subscriptionsMenu/subscribeToNewMovie/:member"
            render={(props) => (
              <SubscribeToNewMovie
                memberName={props.match.params.member}
                movies={movies}
